...
window.compile = function () {
  ...
  try {
    ...
    if (index != input.length) {
      console.log(
        "Anything written after the transition table will be neglected !"
      );
      output.warning =
        "Anything written after the transition table will be neglected !";
    }
...
function next_sym() {
  ...
  if (is_char(current_char)) {
    while (is_char(current_char) || is_num(current_char)) {
      ...
      else if (keyWord.toUpperCase() === "STARTSTATE")
        token = CODE_LEX.START_STATE_TOKEN;
    ...
    if (token === STATE_ID_TOKEN) stateCollector(keyWord); //semantic action
...
function TABLE_ROW() {
  ...
  TABLE_ROW_BODY();
  if (current_sym !== CLOSE_BRACES_TOKEN)
    showError(CLOSE_BRACES_TOKEN, CHARACTER_TOKEN, OPEN_BRACKET_TOKEN);
  current_sym = next_sym();
}

function TABLE_ROW_BODY() {
  if (current_sym === CHARACTER_TOKEN || current_sym === OPEN_BRACKET_TOKEN) {
    finalStates[currentStateIndex].transitions.push(
      new Transition(finalStates[currentStateIndex].stateID)
    );
...
function CHARACTERS() {
  if (current_sym === CHARACTER_TOKEN) {
    finalStates[currentStateIndex].transitions[
      currentTransitionIndex
    ].characters.push(input[index - 2]);
    current_sym = next_sym();
...
function ACTIONS() {
  if (current_sym === RIGHT_TOKEN || current_sym === LEFT_TOKEN) {
    if (current_sym === RIGHT_TOKEN)
      finalStates[currentStateIndex].transitions[
        currentTransitionIndex
      ].direction = "right";
    if (current_sym === LEFT_TOKEN)
      finalStates[currentStateIndex].transitions[
        currentTransitionIndex
      ].direction = "left";
...
function WRITE() {
  if (current_sym === WRITE_TOKEN) {
    ...
    testSym(COLON_TOKEN);
    testSym(CHARACTER_TOKEN);
    finalStates[currentStateIndex].transitions[currentTransitionIndex].write =
      input[index - 3];
    testSym(COMMA_TOKEN);
  } else if (current_sym !== RIGHT_TOKEN && current_sym !== LEFT_TOKEN)
    showError(WRITE_TOKEN, RIGHT_TOKEN, LEFT_TOKEN);
}
...
function TO_STATE() {
  testSym(STATE_ID_TOKEN);
  finalStates[currentStateIndex].transitions[currentTransitionIndex].target =
    dependingStates[dependingStates.length - 1];
}
