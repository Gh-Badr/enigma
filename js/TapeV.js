...
function addTwoCells(blank) {
  ...
  var rightTranslation = rightTranslate
    .match(/translate\(([^,]+)/)[1]
    .split(",");
...
function TapeVisualization(blank, input) {
  ...
  if (Array.isArray(input) && input.length === 0) {
    ...
    if (input.length > Math.floor(cellNumber / 2) + 1) {
      // the input might be length might be larger than the cells provided to it
...
function stepExecute(currentCell, transition, blank) {
  ...
  if (
    ...
    characters.includes(readCharacter(currentCell))
  ) {
...
async function executeAll(
  ...
  var thisCurrentTransition = findTransition(
    transitionTable,
    thisCurrentState,
    currentCell
  );
  ...
  //fire the correspondind transition
  ...
  //check if the execution is stopped by the reset button
  if (!stopFlag) {
    setTimeout(function () {
      if (thisCurrentTransition.target == null)
        executeAll(thisCurrentState, transitionTable, currentCell, blank);
      else {
        currentState = thisCurrentTransition.target;
        executeAll(
          thisCurrentTransition.target,
          transitionTable,
          currentCell,
          blank
        );
  ...
  transitionTable.forEach(function (state) {
    if (state.stateID == currentState) {
      state.transitions.forEach(function (transition) {
        if (
          Array.isArray(transition.characters) &&
          transition.characters.includes(readCharacter(currentCell))
        ) {
